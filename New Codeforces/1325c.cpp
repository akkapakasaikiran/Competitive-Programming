...
int main(){
	...
	int node = -1;
	rep1(i,n) if(a[i].size() >= 3) node = i;

	if(node == -1) rep(i,n-1) cout<<i<<endl;
	else{
		vi labels(n-1,-1);
		int l = 0;
		rep(i,n-1) if((edges[i].first == node || edges[i].second == node) && l <= 2){
			labels[i] = l;
			l++;
		}
		rep(i,n-1) if(labels[i] == -1){
			labels[i] = l;
			l++;
		}
		rep(i,n-1) cout<<labels[i]<<endl;
	} 
